...
| 19 | 2026-07-20 | **MoLE WildChat** DFlash | K=8 r8 experts (rank-64-equiv params) + latent gate (LayerNorm→20480→64→8, softmax, lb-coef 0.01) on the pooled prompt context feature, end-to-end on the FULL unlabeled wild pool (~15k ex, 47 domains, 3 ep); controls comb_r16/comb_r64 (same pool) + 10 own r16 wild specialists; 16-domain wild bench n≤100 | gate COLLAPSED to uniform (per-prompt std ≤0.004, H(sample)=2.07≈ln 8, identical mixture on every domain) — NO emergent latent routing; yet mole is the best mean variant: +0.30pp vs base, ≥ base 16/16, while param-matched comb_r64 is NET NEGATIVE (−0.05pp) ⇒ a uniform ensemble of 8×r8 beats one monolithic r64 (+0.045× pred speedup) — ensemble effect, not specialization. own vs comb_r16 on real wild data: own−comb = +0.018× pred speedup (~2%, range 0..+0.06×) — combined ≈ specialists holds on WildChat too. All wild gains tiny (≤+0.4pp except polish +0.8/+1.0) vs synthetic's +2pp | `experiments/09-mole-wildchat/results/` |
| 18 | 2026-07-20 | **wall-clock speedup** | vanilla target-only baselines on the spec-bench prompts: vLLM (8 EAGLE domains, ~195 tok/s, near-domain-invariant) + HF bf16/sdpa (3 weird domains, ~42 tok/s); speedups collated per framework from committed jsonls | EAGLE multilingual: own speedup > base **5/5** (japanese crosses break-even 0.95→1.02×; best 1.22×); EAGLE weird 1.31–1.68×, LoRA ≈ base; DFlash paired (exp02 baselines) own > base 4/5. **Analytic model validated: speedup ≈ L/(1+c)**, c fitted per speculator/engine (EAGLE-vLLM ≈ 0.18–0.24, DFlash-HF ≈ 0.44) predicts measured wall-clock to 0.3–2% median error on paired/vLLM cells; the one 6%-error section is the unpaired cross-container HF baseline — pair baselines in-container or use the model | `experiments/08-wallclock/results/` |
| 19 | 2026-07-20 | **batched-LoRA serving** | Qwen3-0.6B, zero-delta r16 adapters (identical outputs, pure timing), bs 1→64, paired in-container bases: HF A/B/C (base / unmerged wrappers / merged) + vLLM A/D/E (base / punica ×1 / punica ×50 round-robin) | merged = base within ±1% at every bs (**free**); naive unmerged +20–33%, never amortizes; punica ×1 +58%→+22% (bs 1→64); punica ×50 **+67%→+311%** — overhead scales with DISTINCT adapters live per batch. Confirms the serving recipe: one combined LoRA, merged. Idle vLLM bases spanned 558–900 tok/s across containers — pairing mandatory | `experiments/09-batched-lora-serving/results/` |
