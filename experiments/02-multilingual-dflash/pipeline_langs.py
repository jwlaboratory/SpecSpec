#!/usr/bin/env python3
"""Multilingual LoRA specialization experiment — does specialization work?

Five languages (polish, korean, italian, japanese, german), synthetic prompt sets
(800 train / 100 val / 100 test each). Same one-stack method as ../pipeline.py:
the pretrained z-lab DFlash drafter is trained with SpecForge's OnlineDFlashModel
(DFlash matching loss, target's live hidden states) and benchmarked with its own
spec_generate. Self-distillation: the target Qwen3-8B generates every answer.

Trains SIX rank-16 LoRAs (no full fine-tunes):
  * one per language  (800 examples each)      -> "own" adapters
  * one on all five languages combined (4000)  -> "combined" adapter

Benchmarks a 5x3 matrix over each language's held-out 100-prompt test split:
  base (no adapter)  vs  own-language LoRA  vs  combined LoRA

Specialization is proven if own > base on every language; combined-vs-own shows
...
app = modal.App("dflash-multilingual-lora")
...
PREP = "/work/prep/multilingual"
MODELS = "/work/models/multilingual"
RESULTS = "/work/results/multilingual"


def _read_prompts(lang, split):
    import json
    out = []
    with open(f"/root/data/lang_{lang}/{split}.jsonl", encoding="utf-8") as f:
        for line in f:
            line = line.strip()
            if line:
...
# --------------------------------------------------------------------------- #
# 1) PREP — one vLLM engine, all five languages (train+val) in one container
# --------------------------------------------------------------------------- #
...
def prep(max_new_tokens: int = 256, limit: int = 0):
    ...
    summary = {}
    for lang in LANGS:
        os.makedirs(f"{PREP}/lang_{lang}", exist_ok=True)
        for split in ("train", "val"):
            ...
            for o in outs:
                ...
                records.append({"input_ids": torch.tensor(pids + gids, dtype=torch.long),
                                "prompt_len": len(pids)})
            torch.save(records, f"{PREP}/lang_{lang}/{split}.pt")
            ...
            print(f"[prep:{lang}:{split}] {summary[f'{lang}/{split}']}", flush=True)
    work.commit()
    return summary


# --------------------------------------------------------------------------- #
# 2) TRAIN — one LoRA per dataset: each language alone, plus all combined
# --------------------------------------------------------------------------- #
...
def train_lora(name: str, langs: list, epochs: int = 3, lr: float = 1e-3,
    ...
    print(f"[train:{name}] rank={rank} {len(replaced)} layers, "
          f"{sum(p.numel() for p in trainable):,} LoRA params, langs={langs}", flush=True)
    ...
    train_data, val_data = [], []
    for lang in langs:
        train_data += torch.load(f"{PREP}/lang_{lang}/train.pt")
        val_data += torch.load(f"{PREP}/lang_{lang}/val.pt")
    ...
    def validate():
        online.eval()
        ls, accs = [], []
        # spread the val sample across languages (val_data is ordered by lang)
    ...
    log = {"name": name, "langs": langs, "epochs": epochs, "lr": lr, "val": []}
    online.train()
    step = 0
    t0 = time.time()
...
# --------------------------------------------------------------------------- #
# 3) BENCH — for each language: base vs own-LoRA vs combined-LoRA
# --------------------------------------------------------------------------- #
...
def bench(lang: str, variant: str, max_new_tokens: int = 256, limit: int = 100,
    ...
    with open(out_path, "w") as fout:
        for idx, prompt in enumerate(prompts):
            ...
            recs.append(rec)
            if idx % 20 == 0:
                print(f"[bench:{lang}:{variant}] {idx+1}/{len(prompts)} "
...
# --------------------------------------------------------------------------- #
# aggregate — the 5x3 specialization matrix
# --------------------------------------------------------------------------- #
...
def aggregate():
    ...
    work.reload()
    rows = {}   # (lang, variant) -> metrics
    merged = f"{RESULTS}/multilingual.jsonl"
    ...
    # CSV
    csv = ["lang,variant,n,acceptance_rate_pooled,mean_accept_length,speedup,exact_match_rate"]
    for (lang, variant), d in rows.items():
        csv.append(f"{lang},{variant},{d['n']},{d['accept']:.4f},{d['mean_len']:.4f},"
                   f"{d['speedup']:.4f},{d['exact']:.4f}")
    csv_text = "\n".join(csv) + "\n"

    # Markdown matrix
    md = ["# Multilingual LoRA specialization — base vs own-language LoRA vs combined LoRA\n",
          "Per-language held-out test (n=100 each) · target **Qwen/Qwen3-8B** · drafter "
          "**z-lab/Qwen3-8B-DFlash-b16** · temperature 0 (lossless) · rank-16 LoRA on q/k/v/o.\n",
          "own = LoRA trained on that language only (800 ex); combined = one LoRA trained "
          "on all five languages (4000 ex).\n",
    ...
    md.append(f"\n**own-LoRA beats base on {wins_own}/{len(LANGS)} languages; "
              f"combined LoRA beats base on {wins_comb}/{len(LANGS)}.** "
              "(deltas vs base in parentheses)\n")
    md_text = "\n".join(md)

    with open(f"{RESULTS}/multilingual_report.md", "w") as f:
        f.write(md_text)
    with open(f"{RESULTS}/multilingual_comparison.csv", "w") as f:
        f.write(csv_text)
    work.commit()
    print("\n" + md_text)
...
def orchestrate(epochs: int = 3, bench_limit: int = 100, skip_prep: bool = False,
    ...
    # 6 LoRAs in parallel: 5 individual + 1 combined
    jobs = {lang: train_lora.spawn(lang, [lang], epochs=epochs, rank=rank) for lang in LANGS}
    jobs["combined"] = train_lora.spawn("combined", LANGS, epochs=epochs, rank=rank)
    out["train"] = {name: c.get().get("val_final") for name, c in jobs.items()}
    print(f"[orchestrate] train done: {out['train']}", flush=True)

    # benches in parallel. base is rank-independent: only re-bench it at rank 16
    # (non-default ranks reuse the existing unsuffixed base results).
    variants = VARIANTS if rank == 16 else [v for v in VARIANTS if v != "base"]
    bjobs = {(lang, v): bench.spawn(lang, v, max_new_tokens=bench_max_new_tokens,
...
@app.local_entrypoint()
def smoke():
    """Cheap validation of the new paths: tiny prep (all langs), 1-epoch train of
    ONE individual + the combined (concat path), 2-prompt bench of one language
    across all three variants, aggregate."""
    import json

    print("=== SMOKE prep (8 prompts/lang) ===")
    print(json.dumps(prep.remote(max_new_tokens=96, limit=8), indent=2))
    print("=== SMOKE train polish + combined (1 epoch) ===")
    pj = train_lora.spawn("polish", ["polish"], epochs=1, val_every=1000)
    cj = train_lora.spawn("combined", LANGS, epochs=1, val_every=1000)
    print("[polish]", pj.get().get("saved"), " [combined]", cj.get().get("saved"))
    print("=== SMOKE bench polish x base/own/combined (2 prompts) ===")
    bj = {v: bench.spawn("polish", v, max_new_tokens=64, limit=2, warmup=1)
          for v in VARIANTS}
    for v, c in bj.items():
