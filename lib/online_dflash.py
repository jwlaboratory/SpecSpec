...
Vendored verbatim from SpecForge (specforge/core/dflash.py). The ONLY change is
the line below: the original `from specforge.modeling.draft.dflash import
DFlashDraftModel` is used purely as a type annotation on OnlineDFlashModel.__init__,
so we replace it with a lightweight alias. This makes the training wrapper fully
self-contained (torch only) and lets it wrap ANY DFlashDraftModel-shaped drafter —
in particular the real pretrained `z-lab/Qwen3-8B-DFlash-b16` loaded via AutoModel,
which is the same architecture. Nothing else is modified.
"""
...
DFlashDraftModel = nn.Module  # type-annotation alias only (see module docstring)
